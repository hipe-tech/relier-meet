...
import { SETTINGS_TABS, SettingsButton, openSettingsDialog } from '../../../settings';
import { toggleSharedVideo } from '../../../shared-video';
import { SpeakerStats } from '../../../speaker-stats';
import { TileViewButton, toggleTileView } from '../../../video-layout';
...
class Toolbox extends Component<Props, State> {
    ...
    constructor(props: Props) {
        ...
        this._onToolbarOpenLocalRecordingInfoDialog = this._onToolbarOpenLocalRecordingInfoDialog.bind(
            this
        );
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps._overflowMenuVisible && !prevProps._dialog && this.props._dialog) {
            this._onSetOverflowVisible(false);
    ...
    _isDesktopSharingButtonVisible() {
        const { _desktopSharingEnabled, _desktopSharingDisabledTooltipKey } = this.props;
        ...
        if (isInOverflowMenu) {
            return (
                <OverflowMenuItem
                    ...
                    text = { t(
                        `toolbar.${_screensharing ? 'stopScreenSharing' : 'startScreenSharing'}`
                    ) } />
            );
        }

        const tooltip = t(
            _desktopSharingEnabled ? 'dialog.shareYourScreen' : _desktopSharingDisabledTooltipKey
        );
    ...
    _renderOverflowMenuContent() {
        const { _feedbackConfigured, _fullScreen, _screensharing, _sharingVideo, t } = this.props;
        ...
        return [
            ...
            this._shouldShowButton('e2ee') && <E2EEButton
                key = 'e2ee'
                showLabel = { true } />,
            ...
            this._shouldShowButton('download')
                && <DownloadButton
                    key = 'download'
                    showLabel = { true } />,
            this._shouldShowButton('help') && <HelpButton
                key = 'help'
                showLabel = { true } />
        ];
    }
    ...
    _renderToolboxContent() {
        const { _chatOpen, _hideInviteButton, _overflowMenuVisible, _raisedHand, t } = this.props;
        ...
        if (buttonsRight.length > maxNumberOfButtonsPerGroup) {
            ...
            movedButtons.push(
                ...buttonsRight.splice(numberOfButtons, buttonsRight.length - numberOfButtons)
            );
        }
        ...
        return (
            <div className = 'toolbox-content'>
                <div className = 'button-group-left'>
                    ...
                    {buttonsLeft.indexOf('closedcaptions') !== -1 && <ClosedCaptionButton />}
                </div>
                ...
                <div className = 'button-group-right'>
                    ...
                    {buttonsRight.indexOf('info') !== -1 && <InfoDialogButton />}
...
function _mapStateToProps(state) {
    ...
    const { callStatsID, enableFeaturesBasedOnToken, iAmRecorder } = state['features/base/config'];
    ...
    if (enableFeaturesBasedOnToken) {
        ...
        if (state['features/base/jwt'].isGuest) {
            desktopSharingDisabledTooltipKey = 'dialog.shareYourScreenDisabledForGuest';
        } else {
    ...
    return {
        ...
        _hideInviteButton: iAmRecorder || (!addPeopleEnabled && !dialOutEnabled),
